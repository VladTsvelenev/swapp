  ...
  )
{
  ...
  UINTN SizeOfInfo;
  UINT32 PreferredWidth = 1280;
  UINT32 PreferredHeight = 720;

  for (UINTN i = 0; i < GraphicsOutput->Mode->MaxMode; i++) {
    Status = GraphicsOutput->QueryMode(GraphicsOutput, i, &SizeOfInfo, &Info);
    if (!EFI_ERROR(Status)) {
      if (Info->HorizontalResolution == PreferredWidth && Info->VerticalResolution == PreferredHeight) {
  ...
  )
{
  ...
  //
  // LAB 1: Your code here
  Status = CurrentDriveRoot->Open(CurrentDriveRoot, &KernelFile, (CHAR16 *)KERNEL_PATH, EFI_FILE_MODE_READ, 0);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "JOS: Cannot access own file system - %r\n", Status));
