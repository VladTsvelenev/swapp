...
bind_functions(struct Env *env, uint8_t *binary, size_t size, uintptr_t image_start, uintptr_t image_end) {
    ...
    struct Secthdr *s;
    if (elf->e_shentsize != sizeof(*s) ||
        size < elf->e_shentsize * elf->e_shnum + elf->e_shoff) {
        return -E_INVALID_EXE;
    }
    s = (struct Secthdr *)(binary + elf->e_shoff);
    char *strings = ((char *)binary) + s[elf->e_shstrndx].sh_offset;
    char *strtab = ((char *)binary) + s[elf->e_shstrndx].sh_offset;
    struct Secthdr *st = NULL;
    struct Elf64_Sym *sym = NULL;
    for (int i = 0; i < elf->e_shnum; i++) {
        if (!strcmp(&strings[s[i].sh_name], ".symtab")) {
            st = &s[i];
            continue;
        }
        if (!strcmp(&strings[s[i].sh_name], ".strtab")) {
            strtab = (char *)binary + s[i].sh_offset;
            continue;
        }
    }
    int st_count = st->sh_size / st->sh_entsize;
    for (int j = 0; j < st_count; j++) {
        sym = &((struct Elf64_Sym *)(binary + st->sh_offset))[j];
        ...
        if (sym->st_value >= image_start && sym->st_value <= image_end) {
            *((uintptr_t *)sym->st_value) = (uintptr_t)addr;
        }
    }
...
#ifdef CONFIG_KSPACE
        if (min_address > ph[i].p_va) {
            min_address = ph[i].p_va;
        }

        if (max_address < ph[i].p_va + ph[i].p_memsz) {
            max_address = ph[i].p_va + ph[i].p_memsz;
        }
#endif
...
#ifdef CONFIG_KSPACE
    int err = bind_functions(env, binary, size, min_address, max_address);
    if (err != 0) {
        return -E_INVALID_EXE;
    }
#endif
