...
asan_shadow_allocator(struct UTrapframe *utf) {
    ...
    uintptr_t shadow_addr = (uintptr_t)SHADOW_FOR_ADDRESS(utf->utf_fault_va);
    if (SHADOW_ADDRESS_VALID(shadow_addr)) {
        return 0;
    }

    int res = sys_alloc_region(CURENVID, (void *)shadow_addr, SHADOW_STEP, ALLOC_ONE | PROT_RW);
    return !res;
}
#endif
